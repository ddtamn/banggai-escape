<script lang="ts">
	...
	const heroFacts = [
		{ icon: 'fa-solid fa-location-dot', label: 'WHERE TO?', value: 'Select Destinations' },
...
<!-- Hero -->
<section
	class="relative overflow-hidden px-4 pt-12 pb-20 text-white sm:px-6 md:pt-20 md:pb-28"
	style={heroStyle}
>
	<div class="relative z-10 mx-auto flex max-w-4xl flex-col items-center text-center">
		<div
			class="mb-6 inline-flex items-center gap-2 rounded-full border border-white/15 bg-forest-mid/80 px-3.5 py-1 text-xs font-medium text-stone-100 backdrop-blur-sm md:mb-8"
		>
		...
		>
			Embrace the natural beauty, culture, and heart of Banggai. Your journey begins with Banggai
			Escape.
		</p>

		<form
			class="grid w-full max-w-4xl grid-cols-1 items-center gap-3 rounded-2xl bg-white p-2.5 text-left text-stone-800 shadow-2xl sm:grid-cols-2 sm:gap-3 sm:p-3 lg:grid-cols-12 lg:gap-0 lg:divide-x lg:divide-stone-200 lg:rounded-full lg:px-3 lg:py-1.5"
			action="/packages"
		>
			{#each heroFacts as fact (fact.label)}
				<div class="flex items-center gap-3 border-b border-stone-100 px-3 py-2.5 sm:border-b-0 {fact.label ===
				'WHERE TO?'
					? 'lg:col-span-4'
					: 'lg:col-span-3'}">
					<i class="{fact.icon} text-base text-forest-deep"></i>
					<div>
						<div class="text-[11px] font-bold tracking-wider text-stone-400 uppercase">
							{fact.label}
						</div>
			...
			<div
				class="flex flex-col items-stretch justify-between gap-3 px-3 py-2.5 sm:col-span-2 sm:flex-row sm:items-center lg:col-span-5"
			>
...
<section class="section bg-sand border-y border-stone-100">
	<div class="shell">
		...
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{#each features as feature (feature.title)}
				...
				>
					<div
						class="mx-auto mb-4 flex size-12 items-center justify-center rounded-xl border border-stone-100 bg-stone-50 text-xl text-stone-700"
					>
						<i class={feature.icon}></i>
					</div>
					<h3 class="mb-2.5 text-lg font-bold text-stone-900">{feature.title}</h3>
...
<section class="section-wide bg-white">
	<div class="shell">
		<div class="mb-8 md:mb-12">
			<h2 class="mb-2 text-2xl font-extrabold text-stone-900 md:text-3xl">About us</h2>
		...
		<div class="grid grid-cols-1 items-center gap-8 md:grid-cols-2 lg:gap-12">
			<div class="h-64 w-full overflow-hidden rounded-3xl shadow-sm sm:h-80 md:h-96">
				<img
			...
			<div class="space-y-4 text-xs leading-relaxed text-stone-600 sm:text-sm md:space-y-5">
				<p>
					At Banggai Escape, we are a team of local experts dedicated to sharing the untouched
					wonder of the Banggai Archipelago. Born from a deep passion for our home, we design
					seamless, personalized journeys that showcase vibrant marine life, pristine islands, and
					rich culture—all delivered with authentic warmth, safety, and comfort.
				</p>
				<p>
					Travel is more than visiting a destination; it is about creating unforgettable stories.
					Banggai Escape was founded to bridge curious travelers with Central Sulawesi's most
					breathtaking hidden paradise. With seasoned local guides, flexible itineraries, and
					dedicated support, we ensure every moment of your journey is effortless and
					extraordinary.
				</p>
...
<!-- Testimonials -->
<section class="section-wide border-t border-stone-100 bg-field">
	<div class="shell">
		...
		<div class="mb-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{#each testimonials as testimonial (testimonial.name)}
				...
				>
					<div>
						<div class="mb-3 flex gap-1 text-xs text-gold">
					...
					<div class="flex items-center gap-3 border-t border-stone-100 pt-3">
						...
						/>
						<div>
							<div class="text-xs font-bold text-stone-900">{testimonial.name}</div>
							<div class="text-[11px] text-stone-400">{testimonial.country}</div>
						</div>
					</div>
				</div>
			{/each}
	...
	<div class="shell">
		<div class="grid grid-cols-1 items-start gap-8 md:grid-cols-12 lg:gap-12">
			<div class="md:col-span-7">
				<h2 class="mb-8 max-w-lg text-2xl leading-tight font-extrabold text-stone-900 md:text-3xl">
					Everything you need to know about planning your seamless Banggai experience.
				</h2>
				<Faq items={faqs} />
			</div>
			<div class="h-80 overflow-hidden rounded-3xl shadow-md sm:h-96 md:col-span-5 md:h-[420px]">
				<img
					class="size-full object-cover"
...
<!-- Travel Insights -->
<section class="section bg-sand border-t border-stone-200/60">
	<div class="shell">
		<SectionHeader
